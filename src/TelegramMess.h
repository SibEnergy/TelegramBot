/*
  TelegramMess.h - Library for sending messages to telegram chat
  Created by Oleg N. Andrianov, August 6, 2025.
  Released into the public domain.
*/
#ifndef _TelegramMess_H
#define _TelegramMess_H

class TelegramMess
{
    public:
        void init(char* Token, char* Chat_ID);	// Инициализация
        void send(char* Message);		// Отправка сообщения
    private:
    const char* str_before_token="https://api.telegram.org/bot";
    const char* str_after_token = "/sendMessage?chat_id=";
    const char* str_after_chat_id = "&parse_mode=HTML&text=";
	String str_All;
const char* root_ca=R"string_literal(
-----BEGIN CERTIFICATE-----

4uJEvlz36hz1
-----END CERTIFICATE-----
)string_literal";
};


#endif

